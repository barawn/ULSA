...
c     modified from the subroutine dmdsm        
      subroutine em_los(n_d,l,b,d_max,em1D)
      implicit none
      integer, intent(in):: n_d
      real,intent(out):: em1D(n_d)
      real,intent(in):: l,b,d_max
      real em
...
c wvoid = 1 overrides everything except clumps               
	...
	sm_term = dsm1+dsm2+dsma+dsmgc+dsmlism+dsmcN+dsmvN
        ...
        if(i.ge.2) then
        em1D(i)=em1D(i-1)+sm_factor * dstep * sm_term
        else
        em1D(i)=sm_factor * dstep * sm_term
        endif 
